...
class PathFinder(object):
    ...
    def shortest_path(self, weight):
        ...
        time_used = round(time.process_time() - start_time, 3)
        if path:
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
        inf = float('inf') #for convenience
        Q = PriorityQueue()
        
        #initialization
        for node in nodes:
            node.key = NodeDistancePair(node,inf)
            node.parent = None
            node.visited = False
            if node is source:
                node.key.distance = 0
        ...
        while 0 < len(Q):
            ...
            for u in current.node.adj:
                if u.visited:
                    continue
                
                old_dist = u.key.distance
                new_dist = current.distance + weight(current.node,u)
                
                if new_dist < old_dist:
                    u.key.distance = new_dist
                    u.parent = current.node
                    # u.visited = True
                    
                    Q.decrease_key(u.key)
